...
# set up directories to link to data
dirs = setup_directories()
root = dirs['root']

# read test case
case_name = 'scuc/123bus_case_final.pkl'  # after the generator cost adjustment
...
def run_annual_UC(case_inst,dnum_start,dnum_end):
    for d in range(dnum_start-1,dnum_end):
        ...
        line_annual = np.loadtxt(os.path.join("scuc", 'Line_annual_Dmin.txt'))    # line rating file after the line adjustment [255][365]
        for l in range(case_inst.branchtotnum):
            case_inst.branch.rateA[l] = line_annual[l][d]
        # read solar, wind annual
        solar_folder = os.path.join("scuc", 'solar_annual')
