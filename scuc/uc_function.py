...
### run UC twice: second run will obtain electricity prices
...
# function to build the SCUC case (return pyomo case instance)
def build_UC_full(case_instance,load_b_t):
...
def build_UC_full_Run2(case_instance,UC_case,load_b_t):
    ### Prepare and form UC pyomo data file
    ## list of non wind or solar generators
    reg_gen_list = []
    nonreg_gen_list = []
    ...
    # OH changed from Binary to NonNegativeReals
...
def write_UCresult_day(UC_case, case_nm, day_num):
    ...
    fdpath = os.path.join("scuc", 'UC_results', 'Day' + str(day_num))
