...
def pyomodata_UC(GenericModel,time_totnum,load_b_t,reg_gen_list):    # load_b_t[i][j] where i,j start from 0
    ...
    for i_num in reg_gen_list:
        ...
        gendata_i += str(GenericModel.gen.c0[i])+' '+str(GenericModel.gen.c_su[i])+' '+str(GenericModel.gen.Pmin[i])+' '
    ...
    f.write(time_str)
    i = 0
    while (i < time_totnum):  # 24 hour and initial hour
        time_num = i + 1
    ...
    while (i < bus_totnum):
        bus_num = i + 1
        j = 0
        while (j<time_totnum):    # 24 hour and initial hour
            time_num = j + 1
            bustime_num = k + 1
            bustime_data_i = str(bus_num)+' '+str(time_num)+' '+str(bustime_num)+' '+str(load_b_t[i][time_num-1])+'\n'
